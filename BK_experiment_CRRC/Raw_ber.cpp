...
using namespace std;
	...
	#define B ((float)0.4/(float)1)
	#define C ((float)2.2/(float)3)
	#define D ((float)2.0/(float)3)
	#define E ((float)3.0/(float)3)
	#define F ((float)5.2/(float)3)
	#define G ((float)7.0/(float)3)
#else
...
double BER_before::return_state(int state)
{
	return STATE[state];
}
...
double BER_before::print_state(double STATE[8])
{
	...
	return ((double)STATE[0] * ER + (double)STATE[1] * A + (double)STATE[2] * B + (double)STATE[3] * C + (double)STATE[4] * D + (double)STATE[5] * E + (double)STATE[6] * F + (double)STATE[7] * G) / (double)(totalnum);
}
