...
struct minHeap_6LCNode
{
    ...
    minHeap_6LCNode *E, *D, *C, *B, *A, *Erase; // Left and right child
...
encode_info *score_6LC(const char *path, float accumulate, float alpha)
{
    ...
    encode_info *encode_info_6LC = (encode_info *)malloc(num * sizeof(encode_info));
    for (int i = 0; i < num; i++)
    {
...
{
    return !(root->E) && !(root->D) && !(root->C) && !(root->B) && !(root->A) && !(root->Erase);
}
...
{
    struct minHeap_6LCNode *E = NULL, *D = NULL, *C = NULL, *B = NULL, *A = NULL, *Erase = NULL, *top = NULL;
    ...
    if ((6 % 5) == (total_pattern % 5))
    {
        ...
        minHeap_6LC.pop();
        top = new minHeap_6LCNode(0, E->Freq_6LC + D->Freq_6LC + C->Freq_6LC + B->Freq_6LC + A->Freq_6LC + Erase->Freq_6LC);
        top->E = E;
        top->D = D;
        top->C = C;
        top->B = B;
    ...
    else if ((5 % 5) == (total_pattern % 5))
    {
        ...
        Erase = minHeap_6LC.top();
        minHeap_6LC.pop();
        top = new minHeap_6LCNode(0, D->Freq_6LC + C->Freq_6LC + B->Freq_6LC + A->Freq_6LC + Erase->Freq_6LC);
        top->E = NULL;
        top->D = D;
        top->C = C;
        top->B = B;
    ...
    else if ((4 % 5) == (total_pattern % 5))
    {
        ...
        Erase = minHeap_6LC.top();
        minHeap_6LC.pop();
        top = new minHeap_6LCNode(0, C->Freq_6LC + B->Freq_6LC + A->Freq_6LC + Erase->Freq_6LC);
        top->E = NULL;
        top->D = NULL;
        top->C = C;
        top->B = B;
    ...
    else if ((3 % 5) == (total_pattern % 5))
    {
        ...
        Erase = minHeap_6LC.top();
        minHeap_6LC.pop();
        top = new minHeap_6LCNode(0, B->Freq_6LC + A->Freq_6LC + Erase->Freq_6LC);
        top->E = NULL;
        top->D = NULL;
        top->C = NULL;
        top->B = B;
    ...
    else if ((2 % 5) == (total_pattern % 5))
    {
        ...
        Erase = minHeap_6LC.top();
        minHeap_6LC.pop();
        top = new minHeap_6LCNode(0, A->Freq_6LC + Erase->Freq_6LC);
        top->E = NULL;
        top->D = NULL;
        top->C = NULL;
        top->B = NULL;
...
string decode_file(struct minHeap_6LCNode *root, string s)
{
    ...
    for (int i = 0; i < s.size(); i++)
    {
        ...
        // reached leaf node
        if (curr->E == NULL && curr->D == NULL && curr->C == NULL && curr->B == NULL && curr->A == NULL && curr->Erase == NULL)
        {
            ans += curr->data;
            curr = root;
        }
